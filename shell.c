...
int main() {
    ...
    int cmd_count;
    while (1) {
        /* finTODO:增加打印当前目录，格式类似"shell:/home/oslab ->"，你需要改下面的printf */
        char *cwd=getcwd(NULL,0);
        printf("shell:%s -> ",cwd);
        free(cwd);
        fflush(stdout);
        ...
        if(cmd_count == 0) {
            ...
            int pid = fork();
            if(pid == 0) {  
                /*finTODO:子进程1 将标准输出重定向到管道，注意这里数组的下标被挖空了要补全*/
                close(pipefd[0]);
            ...
            pid = fork();
            if(pid == 0) {  
                /* finTODO:子进程2 将标准输入重定向到管道，注意这里数组的下标被挖空了要补全 */
                close(pipefd[1]);
                dup2(pipefd[0], STDIN_FILENO);
                close(pipefd[0]);

                char *argv[MAX_CMD_ARG_NUM];
                /* finTODO:处理参数，分出命令名和参数，并使用execute运行
                 * 在使用管道时，为了可以并发运行，所以内建命令也在子进程中运行
                 * 因此我们用了一个封装好的execute函数
                 *
                 * 
            ...
            for(int i=0; i<cmd_count; i++) {
                ...
                if(pid == 0) {
                    ...
                    /* finTODO:除了第一条命令外，都将标准输出重定向到上一个管道入口
                     *
                     *
                     * 
                     */
